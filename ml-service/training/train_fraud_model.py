...
Methodology:
    ...
    3. Two feature configurations are evaluated as an ablation study to
       isolate the value of the text channel:
        (a) Numeric-only baseline.
        (b) Numeric + TF-IDF text features.
    4. For each configuration, candidate classifiers are tuned with
       GridSearchCV using 5-fold StratifiedKFold cross-validation
       (scoring=ROC-AUC), with class_weight='balanced' for imbalance.
    5. Per-fold cross-validation scores are reported (mean ± std).
    6. Final test-set evaluation: precision, recall, F1, ROC-AUC, confusion
       matrix.
    7. Best overall pipeline (highest mean CV ROC-AUC) is saved.
    8. Plots: CV ROC-AUC distribution box plot per pipeline, ROC curves on
       test set, confusion matrix of best model.
...
def build_search_specs(include_text: bool) -> dict:
    """
    Per algorithm: (Pipeline, param_grid). HistGradientBoosting is excluded
    from the +TF-IDF config because it does not accept sparse input from the
    TfidfVectorizer.
    """
    specs = {
        "Logistic Regression": (
...
def plot_cv_distribution(results: dict, save_path: str):
    """Box plot of CV ROC-AUC per pipeline (config + model)."""
    ...
    for patch, name in zip(bp["boxes"], names):
        if "Numeric only" in name:
            patch.set_facecolor("#94a3b8")  # gray for baseline
        else:
            patch.set_facecolor("#10b981")  # green for text-augmented
        patch.set_alpha(0.7)
