    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 flex items-start gap-4 shadow-xl">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-600 mt-1">Tenant since {formatDate(company.createdAt)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
          <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center shrink-0 border border-slate-300">
            <Package className="h-5 w-5 text-blue-700" />
          </div>
          <div>
            <p className="text-sm font-medium text-slate-600">Total Products</p>
            <p className="text-2xl font-bold text-slate-900 mt-0.5">{companyProducts.length}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900 mt-0.5">{activePolicies.length}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-slate-600">Open Claims</p>
            <p className="text-2xl font-bold text-slate-900 mt-0.5">{openClaims.length}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900 mt-0.5">{formatCurrency(totalPremium)}</p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-slate-700 mt-1 text-sm leading-relaxed">
                  {company.description || <span className="italic text-slate-500">No description provided.</span>}
                </p>
              </div>
              <div>
                <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Registration Date</p>
                <p className="text-slate-700 mt-1 text-sm">{formatDate(company.createdAt)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-md">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900">Registered Products</h2>
                ...
                <tbody className="divide-y divide-zinc-800">
                  {companyProducts.length > 0 ? (
                    companyProducts.map((prod: any) => (
                      <tr key={prod.id} className="hover:bg-slate-100/30 transition-colors">
                        <td className="px-6 py-4 font-medium text-slate-900">
                          {prod.name}
                        </td>
                        <td className="px-6 py-4">
                          <span className="inline-block bg-slate-100 text-slate-700 text-[10px] font-bold uppercase px-2 py-0.5 rounded border border-slate-300">
                            {prod.type}
                          </span>
                        </td>
