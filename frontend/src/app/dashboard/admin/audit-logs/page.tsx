  ...
  if (result.status === 401 || result.status === 403) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 border-2 border-dashed border-rose-900/30 rounded-3xl bg-rose-950/10 text-center">
        ...
        <p className="text-slate-600 max-w-md">You do not have permission to view the system audit logs.</p>
      </div>
    );
  }
  ...
  if (result.status !== 200 || !result.data) {
    return (
      <div className="text-center py-12">
        <p className="text-slate-500 text-sm">Failed to load audit logs. Please try again later.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-600 mt-1">Platform-wide record of state-changing events.</p>
      </div>
        ...
        <div className="p-0 overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] font-bold tracking-wider">
              <tr>
                <th className="px-6 py-4 border-b border-slate-200">When</th>
                <th className="px-6 py-4 border-b border-slate-200">Action</th>
                <th className="px-6 py-4 border-b border-slate-200">Actor</th>
                <th className="px-6 py-4 border-b border-slate-200">Resource</th>
                ...
                <tr key={log.id} className="hover:bg-slate-100/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      {log.actorRole && (
                        <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest mt-0.5">
                          {log.actorRole}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <details className="group">
                      ...
                      <div className="mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200 max-w-md overflow-x-auto shadow-inner">
