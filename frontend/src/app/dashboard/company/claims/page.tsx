...
export default async function CompanyClaimsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Company Claims</h1>
        <p className="text-slate-600 mt-1 text-sm">Claims filed against your products.</p>
      </div>
            ...
            <table className="w-full text-left text-sm min-w-[800px]">
              <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] font-bold tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right">Fraud Score</th>
                  <th className="px-6 py-3 text-right">Filed Date</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={claim.id} className="hover:bg-slate-100/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <p className="text-slate-700 font-medium">{claim.user?.firstName} {claim.user?.lastName}</p>
                        <p className="text-[10px] text-slate-500">{claim.application?.product?.name}</p>
                      </td>
