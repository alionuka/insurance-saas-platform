...
export default async function CompanyClaimsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white">Company Claims</h1>
        <p className="text-zinc-400 mt-1 text-sm">Claims filed against your products.</p>
      </div>
                  ...
                  return (
                    <tr key={claim.id} className="hover:bg-zinc-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <p className="text-zinc-300 font-medium">{claim.user?.firstName} {claim.user?.lastName}</p>
                        <p className="text-[10px] text-zinc-500">{claim.application?.product?.name}</p>
                      </td>
