...
export default async function ClientApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      {/* Hero */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 flex items-start gap-4 shadow-xl">
        ...
        <div className="flex-1">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-600 mt-1">Review your application details and status.</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-md">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold text-slate-900">Product Overview</h2>
          </div>
          {product ? (
            <div className="space-y-4">
              <div>
                <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Insurance Plan</p>
                <p className="text-slate-900 font-medium text-lg mt-1">{product.name}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Type</p>
                  <p className="text-slate-700 mt-1 uppercase font-bold text-sm bg-slate-100 inline-block px-2 py-0.5 rounded">{product.type}</p>
                </div>
                <div>
              ...
              {product.company && (
                <div className="pt-4 border-t border-slate-200/50">
                  <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Underwriter</p>
                  <p className="text-slate-700 mt-1 font-medium">{product.company.name}</p>
                </div>
              )}
            </div>
          ) : (
          ...
          {risk ? (
            <div className="flex-1 flex flex-col">
              <div className="flex items-end gap-3 mb-4">
                <span className="text-4xl font-bold font-mono text-slate-900 leading-none">
                  {risk.riskScore.toFixed(0)}
                </span>
                <span className="text-sm text-slate-500 mb-1">/ 100 score</span>
              </div>
              <div className="mb-4">
                <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase border bg-slate-50 ${getRiskLevelColor(risk.riskLevel)} border-current/20`}>
                  {risk.riskLevel} Risk
                </span>
              </div>
              <div className="mt-auto p-4 bg-slate-50/50 rounded-lg border border-slate-200">
        ...
        {/* Linked Policy Card */}
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-md md:col-span-2 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        {/* Timeline Card */}
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-md md:col-span-2">
          <div className="flex items-center gap-2 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 bg-slate-50/50 rounded-lg border border-slate-200">
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Submitted On</p>
              <p className="text-slate-700 mt-1 font-medium">{formatDate(app.createdAt)}</p>
            </div>
            <div className="p-4 bg-slate-50/50 rounded-lg border border-slate-200">
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Last Updated</p>
              <p className="text-slate-700 mt-1 font-medium">{formatDate(app.updatedAt)}</p>
            </div>
          </div>
        </div>
      </div>
