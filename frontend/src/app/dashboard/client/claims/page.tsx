...
export default async function ClientClaimsPage(props: Props) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{t('clientClaims.title')}</h1>
        <p className="text-slate-600 dark:text-slate-400 mt-1 text-sm">{t('clientClaims.subtitle')}</p>
      </div>
          ...
          claims.map((claim: any) => {
            ...
            return (
              <Link key={claim.id} href={`/dashboard/client/claims/${claim.id}`} className="block group">
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden group-hover:border-slate-300 transition-colors">
                  <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex-grow">
                      <h3 className="font-bold text-slate-900 dark:text-slate-100 text-lg">{claim.application?.product?.name || t('sidebar.nav.claims')}</h3>
                      <p className="text-sm text-slate-700 dark:text-slate-300 mt-1 font-medium">{claim.description}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 italic">{t('clientClaims.claimId')}: {claim.id.substring(0, 8)} • {t('clientClaims.amount')}: {formatCurrency(claim.amount)} • {t('clientClaims.filed')}: {formatDate(claim.createdAt)}</p>
                    </div>
                  </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                      <div className="md:col-span-1 p-3 rounded-lg bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold">{t('clientClaims.fraudScore')}</span>
                        </div>
                        <p className="text-lg font-mono font-bold text-slate-900 dark:text-slate-100">{fraud.fraudScore.toFixed(1)}<span className="text-xs text-slate-500 dark:text-slate-400 ml-1">/ 100</span></p>
                      </div>
                      <div className="md:col-span-1 p-3 rounded-lg bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
                        <div className="flex items-center gap-2 mb-1">
                          <AlertCircle className="h-3 w-3 text-slate-500 dark:text-slate-400" />
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold">{t('clientClaims.fraudFlag')}</span>
                        </div>
                        <p className={`text-lg font-bold ${
                          fraud.flag === 'NORMAL' ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400 animate-pulse'
                        }`}>{fraud.flag}</p>
                      </div>
                      <div className="md:col-span-2 p-3 rounded-lg bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold">{t('clientClaims.mlAssessment')}</span>
                        </div>
                        <p className="text-xs text-slate-600 dark:text-slate-400 italic leading-relaxed">"{fraud.explanation}"</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="px-5 pb-4 border-t border-slate-200 dark:border-slate-800/50">
                  <details className="group">
                    <summary className="list-none cursor-pointer py-2 flex items-center gap-2 text-[10px] text-slate-500 dark:text-slate-400 hover:text-blue-700 uppercase font-bold tracking-tight transition-colors">
