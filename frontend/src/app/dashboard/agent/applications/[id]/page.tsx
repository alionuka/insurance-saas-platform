      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-400 mt-1">Agent Investigation Workspace</p>
          </div>
        </div>
        
        {/* Status Update Widget */}
        <div className="bg-zinc-950/50 border border-zinc-800 p-4 rounded-xl flex flex-col items-start gap-2">
          <span className="text-xs font-bold uppercase text-zinc-500 tracking-wider">Update Status</span>
            ...
            {user ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-white font-medium text-lg mt-1">{user.firstName} {user.lastName}</p>
                  <p className="text-sm text-zinc-400">{user.email}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-zinc-500 uppercase tracking-wider font-semibold">Age</p>
                    <p className="text-zinc-300 mt-1 font-mono text-lg">{user.age ?? 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-zinc-500 uppercase tracking-wider font-semibold">Credit Score</p>
                    <p className="text-zinc-300 mt-1 font-mono text-lg">{user.creditScore ?? 'N/A'}</p>
                  </div>
                </div>
              </div>
            ) : (
              <p className="text-sm text-zinc-500 italic">No customer details available.</p>
            )}
          </div>

          {/* Linked Policy Card */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
            ...
            {risk ? (
              <div className="flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-4 pb-4 border-b border-zinc-800">
                  ...
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase border bg-zinc-950 ${getRiskLevelColor(risk.riskLevel)} border-current/20`}>
                    {risk.riskLevel}
                  </span>
                </div>
            ...
            {product ? (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-zinc-300 mt-1 uppercase font-bold text-xs bg-zinc-800 inline-block px-2 py-0.5 rounded">{product.type}</p>
                </div>
                <div>
