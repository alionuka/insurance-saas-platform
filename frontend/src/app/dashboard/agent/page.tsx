...
export default async function AgentOverviewPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">{t('dashboard.agentTitle')}</h1>
        <p className="text-slate-600 mt-1 text-sm">{t('dashboard.agentSubtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-medium text-slate-700">{t('dashboard.pendingApps')}</h3>
          </div>
          <p className="text-3xl font-bold text-slate-900">{pendingApps}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-2">
            <Activity className="h-5 w-5 text-emerald-400" />
            <h3 className="font-medium text-slate-700">{t('dashboard.stats.openClaims')}</h3>
          </div>
          <p className="text-3xl font-bold text-slate-900">{openClaims}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-2">
            <ShieldAlert className="h-5 w-5 text-rose-400" />
            <h3 className="font-medium text-slate-700">High-Risk Apps</h3>
          </div>
          <p className="text-3xl font-bold text-slate-900">{highRiskApps}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-2">
            <AlertTriangle className="h-5 w-5 text-blue-700" />
            <h3 className="font-medium text-slate-700">Suspicious Claims</h3>
          </div>
          <p className="text-3xl font-bold text-slate-900">{suspiciousClaims}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6">
          <h2 className="text-lg font-medium text-slate-900 mb-4">{t('dashboard.recentActivity')}</h2>
          <div className="space-y-4">
            {allActivity.map((item: any) => (
              <div key={`${item.type}-${item.id}`} className="flex items-start gap-4 p-4 rounded-lg bg-slate-50/50 border border-slate-200/50">
                <div className="h-10 w-10 rounded-full bg-white border border-slate-300 flex items-center justify-center shrink-0">
                  {item.type === 'APPLICATION' ? <FileText className="h-5 w-5 text-blue-700" /> : <Activity className="h-5 w-5 text-emerald-400" />}
                </div>
                ...
                <Link 
                  ...
                  className="h-8 w-8 rounded-full hover:bg-slate-100 flex items-center justify-center text-slate-500 hover:text-slate-900 transition-colors"
                >
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          <h2 className="text-lg font-medium text-slate-900 mb-4">Risk Distribution</h2>
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-emerald-400 font-medium">Low Risk</span>
                <span className="text-slate-600">{lowRisk}</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-600">{medRisk}</span>
              </div>
              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-600">{highRisk}</span>
              </div>
              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
