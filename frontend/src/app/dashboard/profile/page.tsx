...
export default async function ProfilePage() {
  ...
  if (error === 'UNAUTHORIZED') {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-900 mb-2">{t('profile.accessDenied')}</h2>
        <p className="text-slate-600">{t('profile.accessDeniedHint')}</p>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <AlertCircle className="h-12 w-12 text-rose-500 mb-4" />
        <h2 className="text-xl font-bold text-slate-900 mb-2">{t('profile.errorLoading')}</h2>
        <p className="text-slate-600">{t('profile.errorLoadingHint')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl space-y-8 pb-12">
      {/* Hero Section */}
      <div className="flex flex-col md:flex-row md:items-center gap-6 bg-white border border-slate-200 p-8 rounded-xl relative overflow-hidden">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">{user.firstName} {user.lastName}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-8">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-slate-200/60">
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('profile.emailAddress')}</p>
                <p className="text-slate-900 font-medium break-all">{user.email}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('profile.role')}</p>
                <p className="text-slate-900 font-medium">{user.role}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('profile.memberSince')}</p>
                <p className="text-slate-900 font-medium">{formatDate(user.createdAt)}</p>
              </div>
              {user.role === 'COMPANY_ADMIN' && user.company && (
                <div>
                  <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">{t('profile.companyAffiliation')}</p>
                  <p className="text-slate-900 font-medium">{user.company.name}</p>
                </div>
              )}
            </div>

            {user.role === 'CUSTOMER' && (
              <div className="grid grid-cols-3 gap-3 mb-6 pb-6 border-b border-slate-200/60">
                <div className="bg-blue-700/5 border border-blue-700/20 rounded-lg p-3">
                  ...
                  <p className="text-slate-900 font-semibold text-lg">{user.age ?? '—'}</p>
                  <p className="text-[9px] text-slate-500 mt-1">used by Risk ML</p>
                </div>
                <div className="bg-blue-700/5 border border-blue-700/20 rounded-lg p-3">
                  <p className="text-[10px] text-blue-700 uppercase font-bold tracking-wider mb-1">Credit Score</p>
                  <p className="text-slate-900 font-semibold text-lg">{user.creditScore ?? '—'}</p>
                  <p className="text-[9px] text-slate-500 mt-1">used by Risk ML</p>
                </div>
                <div className="bg-blue-700/5 border border-blue-700/20 rounded-lg p-3">
                  <p className="text-[10px] text-blue-700 uppercase font-bold tracking-wider mb-1">Annual Income</p>
                  <p className="text-slate-900 font-semibold text-lg">{user.annualIncome != null ? `$${Number(user.annualIncome).toLocaleString()}` : '—'}</p>
                  <p className="text-[9px] text-slate-500 mt-1">used by Risk + Recs</p>
                </div>
              </div>
            )}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6">
            ...
            <p className="text-xs text-slate-500 mb-6">{t('profile.securityHint')}</p>
            ...
            {auditLogs && auditLogs.length > 0 ? (
              <div className="divide-y divide-zinc-800/50">
                {auditLogs.map((log: { id: string; action: string; resourceType?: string; resourceId?: string; createdAt: string }) => (
                  <div key={log.id} className="p-5 hover:bg-slate-100/20 transition-colors">
                    ...
                    {log.resourceType && (
                      <p className="text-sm text-slate-600 font-medium">
                        {log.resourceType} <span className="text-slate-400 font-normal">#{log.resourceId?.substring(0, 8)}</span>
                      </p>
                    )}
                  </div>
                ))}
              ...
              <div className="p-12 text-center">
                <p className="text-slate-500">{t('profile.noActivity')}</p>
              </div>
            )}
          </div>
        </div>
