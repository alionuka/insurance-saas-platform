...
export default async function ProfilePage() {
  ...
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      ...
      case 'COMPANY_ADMIN': return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
    ...
    <div className="max-w-4xl space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-6 bg-zinc-900 border border-zinc-800 p-8 rounded-xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center gap-3 shrink-0">
          <div className="h-24 w-24 rounded-full bg-blue-500/20 border border-blue-500/30 flex items-center justify-center overflow-hidden">
            {user.avatarUrl ? (
              <img
                src={user.avatarUrl}
              ...
              <span className="text-3xl font-bold text-blue-400">{getInitials()}</span>
            )}
          </div>
          <AvatarUploadButton />
        </div>
            ...
            {user.role === 'CUSTOMER' && (
              <div className="grid grid-cols-3 gap-3 mb-6 pb-6 border-b border-zinc-800/60">
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg p-3">
                  <p className="text-[10px] text-blue-400 uppercase font-bold tracking-wider mb-1">Age</p>
                  <p className="text-white font-semibold text-lg">{user.age ?? '—'}</p>
                  <p className="text-[9px] text-zinc-500 mt-1">used by Risk ML</p>
                </div>
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg p-3">
                  <p className="text-[10px] text-blue-400 uppercase font-bold tracking-wider mb-1">Credit Score</p>
                  <p className="text-white font-semibold text-lg">{user.creditScore ?? '—'}</p>
                  <p className="text-[9px] text-zinc-500 mt-1">used by Risk ML</p>
                </div>
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-lg p-3">
                  <p className="text-[10px] text-blue-400 uppercase font-bold tracking-wider mb-1">Annual Income</p>
