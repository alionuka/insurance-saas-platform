  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 shadow-2xl text-left">
      ...
      <p className="text-xs text-zinc-400 mt-1">
        Cumulative impact: <span className="text-amber-400 font-mono font-bold">{data.totalImpact.toFixed(1)} pts</span>
      </p>
          ...
          >
            ...
            <Bar dataKey="totalImpact" radius={4} barSize={14}>
              {chartData.map((_, index) => (
                <Cell key={`cell-${index}`} fill="#fbbf24" fillOpacity={0.85} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
