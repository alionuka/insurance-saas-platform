...
export default function Skeleton({ className = '' }: SkeletonProps) {
  return (
    <div
      aria-hidden
      className={`relative overflow-hidden bg-slate-100 dark:bg-slate-800/60 rounded-lg ${className}`}
    >
...
export function SkeletonCard() {
  return (
    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-xl p-6 space-y-4">
...
export function SkeletonTable({
  ...
  return (
    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
      <div
        className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 grid gap-4"
      ...
      {Array.from({ length: rows }).map((_, rowIdx) => (
        <div
          key={rowIdx}
          className="px-6 py-4 border-b border-slate-200 dark:border-slate-800/40 grid gap-4 last:border-b-0"
          style={{ gridTemplateColumns: `repeat(${columns}, 1fr)` }}
        >
